...
export default function ForgotPasswordScreen({ navigation }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await fetch("http://192.168.55.105:3101/forgot-password", {
        method: "POST",
        ...
        <Animatable.View animation="fadeInDown" duration={800} style={styles.container}>
          ...
          <Animatable.View animation="fadeInUp" delay={400} style={styles.bottomContainer}>
            ...
            <Animatable.Text animation="fadeIn" delay={800} style={styles.description}>
              Enter your email address to receive instructions for creating a new password.
            </Animatable.Text>
...
const styles = StyleSheet.create({
  ...
  successMessageContainer: {
    ...
    backgroundColor: '#4BB543', paddingVertical: 10, paddingHorizontal: 20,
    zIndex: 9999, elevation: 10, alignItems: 'center',
  },
  successMessageText: { color: '#fff', fontWeight: 'bold', fontSize: 14 },
  topContainer: {
    flex: 1, borderBottomLeftRadius: 60, borderBottomRightRadius: 60, overflow: 'hidden',
  },
  ...
  bottomContainer: { flex: 1.2, padding: 25, justifyContent: 'center', backgroundColor: '#fff' },
  backButton: { position: 'absolute', top: 20, left: 20, zIndex: 1 },
  title: { fontSize: 24, fontWeight: 'bold', color: '#000', textAlign: 'center', marginTop: -50, marginBottom: 10 },
  description: { fontSize: 14, color: '#333', textAlign: 'center', marginBottom: 30 },
  inputWrapper: {
    flexDirection: 'row', alignItems: 'center', backgroundColor: '#eee',
    borderRadius: 30, paddingHorizontal: 15, marginBottom: 5,
  },
  icon: { marginRight: 10 },
  input: { flex: 1, height: 50, color: '#000' },
  errorText: { color: 'red', fontSize: 12, marginLeft: 15, marginBottom: 10 },
  button: {
    backgroundColor: '#FFAC1C', borderRadius: 30, paddingVertical: 15,
