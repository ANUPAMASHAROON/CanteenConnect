    ...
    navigation.setOptions({
      ...
      headerTitle: () => (
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Image
            source={require('../assets/aparna.jpg')}
  ...
  const fetchFoodItems = async () => {
    try {
      const response = await fetch(
        'https://dev285691.service-now.com/api/now/table/u_canteen_menu?sysparm_query=u_canteen_name=Aparna Canteen',
        {
          method: 'GET',
          headers: {
      ...
      const formattedItems = data.result.map(item => ({
        ...
        canteen: item.u_canteen_name || 'Aparna Canteen',
  ...
  const addToCart = async item => {
    try {
      ...
      const newItem = {
        ...
        canteen_name: item.canteen || 'Aparna Canteen',
...
export default AparnaCanteenPage;
