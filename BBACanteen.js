    ...
    navigation.setOptions({
      ...
      headerTitle: () => (
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          <Image
            source={require('../assets/BBAcanteen.jpg')}
  ...
  const fetchFoodItems = async () => {
    try {
      const response = await fetch(
        'https://dev285691.service-now.com/api/now/table/u_canteen_menu?sysparm_query=u_canteen_name=BBA Canteen',
        {
          method: 'GET',
          headers: {
      ...
      const formattedItems = data.result.map(item => ({
        ...
        canteen: item.u_canteen_name || 'BBA Canteen',
  ...
  const addToCart = async (item) => {
    try {
      ...
      const newItem = {
        ...
        canteen_name: item.canteen || "BBA Canteen",
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 10 }}>
          {ITEM_TYPES.map(type => (
            <TouchableOpacity
              ...
              <Text style={[themedStyles.filterText, selectedType === type && themedStyles.activeFilterText]}>
                {type}
              </Text>
            </TouchableOpacity>
          ))}
              ...
              <View key={item.id} style={themedStyles.card}>
                ...
                <TouchableOpacity
                  ...
                  <MaterialIcons
                    ...
                    size={24}
                    color={isFavorited ? '#FF8000' : '#aaa'}
                  />
                </TouchableOpacity>

                <TouchableOpacity
                  style={[themedStyles.button, { marginTop: 8 }, isOutOfStock && themedStyles.disabledButton]}
