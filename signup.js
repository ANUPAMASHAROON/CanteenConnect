...
//   const base_url = "http://10.244.17.79:3101/signup";
...
export default function SignupScreen({ navigation }) {
  ...
  const validateAndSubmit = async () => {
    ...
    if (Object.keys(newErrors).length === 0) {
      try {
        const response = await axios.post('http://192.168.55.105:3101/signup', {
          username,
          email,
          password,
        });
...
const styles = StyleSheet.create({
  ...
  backButton: { position: 'absolute', top: 25, left: 20, zIndex: 10 },
  backIconWrapper: {
    backgroundColor: '#fff',
    borderRadius: 20,
    padding: 5,
  ...
  tabActive: { marginTop: -10, fontSize: 26, color: '#000', fontWeight: 'bold' },
  inputWrapper: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#fff',
  ...
  errorText: { color: '#8B0000', fontSize: 13, marginBottom: 8, marginLeft: 12 },
  button: {
    backgroundColor: '#fff',
    borderRadius: 30,
    height: 50,
  ...
  buttonText: { color: '#FFAC1C', fontWeight: 'bold', fontSize: 16 },
  orContainer: { flexDirection: 'row', alignItems: 'center', marginVertical: 18 },
  line: { flex: 1, height: 1, backgroundColor: '#fff' },
  orText: { marginHorizontal: 10, color: '#fff', fontWeight: 'bold' },
  socialButton: {
    flexDirection: 'row',
    alignItems: 'center',
    backgroundColor: '#fff',
  ...
  bottomLink: { textAlign: 'center', color: '#000', marginTop: 10, fontSize: 14 },
